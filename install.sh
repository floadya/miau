...
VALIDITY () {
    ...
    Exp1=$(curl https://github.com/floadya/miau/blob/main/install.sh | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl https://github.com/floadya/miau/blob/main/install.sh | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://github.com/floadya/miau/blob/main/install.sh | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
# validity
rm -f /usr/bin/e
valid=$(curl https://github.com/floadya/miau/blob/main/install.sh | grep $MYIP | awk '{print $4}')
...
today=`date -d "0 days" +"%Y-%m-%d"`
Exp1=$(https://github.com/floadya/miau/blob/main/install.sh | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://github.com/floadya/miau/"
