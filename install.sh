...
VALIDITY () {
    ...
    Exp1=$(curl https://raw.githubusercontent.com/CatzBurry/AIO/main/access/ip | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl https://raw.githubusercontent.com/CatzBurry/AIO/main/access/ip | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/CatzBurry/AIO/main/access/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
# validity
rm -f /usr/bin/e
valid=$(curl https://raw.githubusercontent.com/CatzBurry/AIO/main/access/ip | grep $MYIP | awk '{print $4}')
...
today=`date -d "0 days" +"%Y-%m-%d"`
Exp1=$(https://raw.githubusercontent.com/CatzBurry/AIO/main/access/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://github.com/CatzBurry/AIO/"
